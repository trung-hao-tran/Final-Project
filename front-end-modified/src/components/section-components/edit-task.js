...
const convertDateTimeToLocal = (dateTimeStr) => {
    ...
    return date.toISOString().slice(0, 16); // 提取 YYYY-MM-DDTHH:mm 部分
};
...
const EditTask = () => {
    ...
    // 设置默认值
    if (!id) {
        id = "654d9453e1386de1f1b1642d"; // 替换为您希望的默认值
    }
    ...
    const onSubmitUserClicked = async (e) => {
        ...
        if (canSave) {
            ...
            // 调用updateTask mutation进行更新
            try {
                const result = await updateTask({ id, ...updatedTask });
                if (result.data) {
                    // 处理成功更新后的逻辑，比如显示通知或重定向
                    toast.success("Task updated successfully");
                    navigate("/tasks"); // 假设您在更新后想要导航到任务列表
                }
            } catch (error) {
                // 处理错误情况
